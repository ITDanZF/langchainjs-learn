...
`ask` 子命令只适合教学早期验证模型调用。第 03 章开始不再把它作为用户主入口。

```text
第 01-02 章：ask 子命令验证模型是否接通
第 03 章之后：npm run dev 默认进入交互会话
```
...
现在交互式输入的每一轮都会流式输出。后续如果确实需要调试非流式结果，可以再单独加内部调试命令，不放进主线交互方式里。
